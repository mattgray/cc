...
Mobile = {
	...
	Init: function() {
		...
		// Re-init lightbox after setting options
		...
		return this.each(function() {
			...
			this.Swipe = function(event, phase, direction, distance) {
				...
				if (phase=="move" && (direction=="left" || direction=="right")) {
					...
					// If  there's no detected direction, we launch the lightbox
					} else if ( direction ===null) {
						$('img', Self).eq(Self.Data.CurrentImg).click();
					}
				} else if ( phase == "click" ) {
					// Progress if we've clicked
					// This phase is not triggered in iOS
// 					Self.NextImg();
				}
			};
